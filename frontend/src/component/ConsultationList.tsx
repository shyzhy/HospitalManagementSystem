...
const ConsultationList: React.FC<ConsultationListProps> = ({ patients, onUpdate }) => {
    ...
    const filteredConsultations = consultations.filter(c => {
        ...
        if (userRole === 'doctor') {
            if (activeTab === 'pending') return matchesSearch && !c.diagnosis;
            if (activeTab === 'completed') return matchesSearch && c.diagnosis;
        }
    ...
    return (
        <div className="space-y-6 px-4 pt-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                {/* DIAGNOSTIC TABS (Doctor Only) */}
                {userRole === 'doctor' && (
                    <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 shadow-sm self-start md:self-auto">
                        <button 
                            onClick={() => setActiveTab('pending')}
                            className={`px-6 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'pending' ? 'bg-white text-[#556ee6] shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                            Pending
                        </button>
                        <button 
                            onClick={() => setActiveTab('completed')}
                            className={`px-6 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'completed' ? 'bg-white text-[#556ee6] shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                            Completed
                        </button>
                    </div>
                ...
                <table className="w-full text-left min-w-[500px]">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {filteredConsultations.length === 0 ? (
                            <tr><td colSpan={4} className="px-6 py-20 text-center text-slate-300 font-bold text-xs uppercase tracking-[0.2em]">No {activeTab} clinical encounters archived</td></tr>
                        ) : (
