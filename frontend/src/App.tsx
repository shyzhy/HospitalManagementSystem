...
function AppContent() {
  ...
  // Allow explicit unauthenticated access to login and activation routes
  ...
  // Redirect to login if user is unauthenticated and attempting to access internal panels
  if (!token) {
    return <Navigate to="/login" replace />;
  }
        ...
        <main className="flex-1 overflow-y-auto p-3 sm:p-5 lg:p-8 relative">
          <div className="bg-transparent min-h-[60vh]">
            <Routes>
              ...
              {/* Fallback route within application wrapper */}
